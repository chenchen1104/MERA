...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.moe_norm = nn.LayerNorm(hidden_size//8)
    ...
    def forward(self, x, similars):
        ...
        similar_features = similars[:,:,:-2]
        attn_weights_feature = F.softmax(torch.bmm(x, similar_features.transpose(1, 2)), dim=-1) 

        similar_label = similars[:,:,-2] 
        
        similar_label = self.embedding(similar_label.int())
        weighted_similar_label = torch.bmm(attn_weights_feature, similar_label) # torch.Size([bs, 1, 128])
        weighted_similar_label = weighted_similar_label.squeeze(1)
        output = self.moe(inp = out.permute(1, 0, 2), gate_inp = weighted_similar_label)    
